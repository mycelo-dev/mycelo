...
func TestSelectQueries(t *testing.T) {
	...
	assertQueryContainsAll(t, select_queries.GetEventsAfterCursorQuery(), "FROM events", "created_at > $2", "ORDER BY created_at ASC, id ASC")
	...
	assertQueryContainsAll(t, select_queries.GetOutboundMappingStateQuery(), "destination_address", "retry_base_delay_ms", "skip_on_endpoint_transport_error", "last_skipped_event_id", "last_error_category", "last_error")
