...
func TestInsertQueries(t *testing.T) {
	...
	assertQueryContainsAll(t, insert_queries.GetInsertOutboundEventDeliveryQuery(), "INSERT INTO outbound_event_deliveries", "unnest($3::bigint[])", "status", "DO NOTHING")
...
func TestSelectQueries(t *testing.T) {
	...
	assertQueryContainsAll(t, select_queries.GetDestinationTopicMappingsByTenantAndTeamQuery(), "FROM destination_topic_mapping dtm", "retry_base_delay_ms", "dead_letter_queue_enabled", "last_skipped_event_id", "last_error_category", "topic_name", "MAX(e.id)", "outbound_event_deliveries")
	...
	assertQueryContainsAll(t, select_queries.GetEventsAfterCursorQuery(), "FROM events", "tenant_public_id = $1", "team_public_id = $2", "id > $5", "ORDER BY id ASC", "LIMIT $6")
	...
	assertQueryContainsAll(t, select_queries.GetTopicHeadsByTenantAndTeamQuery(), "FROM topics t", "LEFT JOIN events e", "COALESCE(MAX(e.id), 0)", "GROUP BY t.topic_public_id, t.topic_name")
	...
	assertQueryContainsAll(t, select_queries.GetClaimOutboundEventDeliveriesQuery(), "FOR UPDATE SKIP LOCKED", "attempt_count = attempt_count + 1", "RETURNING")
...
func TestUpdateQueries(t *testing.T) {
	...
	assertQueryContainsAll(t, update_queries.GetUpdateUnorderedEnqueueCursorQuery(), "unordered_last_enqueued_event_id", "delivery_lease_holder = $4")
	assertQueryContainsAll(t, update_queries.GetMarkOutboundEventDeliveryDeliveredQuery(), "status = 'delivered'", "locked_by = $2")
